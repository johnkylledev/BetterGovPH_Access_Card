      ...
      >
        ...
        <div className="absolute bottom-6 right-6 w-12 h-12 rounded-full overflow-hidden pointer-events-none z-30 shadow-[0_0_10px_rgba(255,255,255,0.05)] border border-white/20"
          ...
          <div className="absolute inset-0 opacity-[0.3]">
            <svg viewBox="0 0 100 100" className="w-full h-full">
              <defs>
                <pattern id="microInterference" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
                  {Array.from({ length: 12 }).map((_, i) => (
                    <g key={`micro-${i}`}>
                      <path
                        d={`M 0 ${i * 1.6} C 5 ${i * 1.6 - 3}, 15 ${i * 1.6 + 3}, 20 ${i * 1.6}`}
                        fill="none"
                        stroke="#ffffff"
                        strokeWidth="0.1"
                        opacity={0.3 + (i % 2) * 0.4}
                      />
                      <path
                        d={`M ${i * 1.6} 0 C ${i * 1.6 - 3} 5, ${i * 1.6 + 3} 15, ${i * 1.6} 20`}
                        fill="none"
                        stroke="#ffffff"
                        strokeWidth="0.1"
                        opacity={0.2 + (i % 2) * 0.4}
        ...
        <div className="relative z-10 flex flex-col items-center w-full" style={{ transform: 'translateZ(20px)' }}>
          ...
          <div className="text-center">
            <p className="text-xs font-sans tracking-[0.25em] text-white font-semibold mt-1">BetterGovPH</p>
          </div>
        </div>
